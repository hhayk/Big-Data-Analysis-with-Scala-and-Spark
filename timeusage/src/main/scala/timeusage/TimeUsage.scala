...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def ag = Seq(
      Seq("t01", "t03", "t11", "t1801", "t1803"),
      Seq("t05", "t1805"),
      Seq("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    )

    def rec(primary:List[Column], working:List[Column], other:List[Column], cn: List[String]): (List[Column], List[Column], List[Column]) = cn match {
      case Nil => (primary, working, other)
      case x::xs => {
        x match {
          case _ if ag(0).exists(m => x.startsWith(m)) => rec(col(x) :: primary, working, other, xs)
          case _ if ag(1).exists(m => x.startsWith(m)) => rec(primary, col(x) :: working, other, xs)
          case _ if ag(2).exists(m => x.startsWith(m)) => rec(primary, working, col(x) :: other, xs)
          case _ => rec(primary, working, other, xs)
        }
      }
    }

    rec(List(), List(), List(), columnNames)
  }
  ...
  def timeUsageSummary(
    ...
    def sumInHours(columns:List[Column]) = columns.foldLeft(lit(0))((acc, col) => acc + df(col.toString())) / 60.0
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(t => TimeUsageRow(t.getAs("working"), t.getAs("sex"), t.getAs("age"), t.getAs("primaryNeeds"), t.getAs("work"), t.getAs("other")))
